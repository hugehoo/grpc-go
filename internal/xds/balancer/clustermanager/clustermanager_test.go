...
"children": {
	...
	if err := bal.UpdateClientConnState(balancer.ClientConnState{
		ResolverState: resolver.State{Endpoints: []resolver.Endpoint{
			hierarchy.SetInEndpoint(resolver.NewEndpoint(wantAddrs[0]), []string{"cds:cluster_1"}),
			hierarchy.SetInEndpoint(resolver.NewEndpoint(wantAddrs[1]), []string{"cds:cluster_2"}),
		}},
...
"children": {
	...
	if err := bal.UpdateClientConnState(balancer.ClientConnState{
		ResolverState: resolver.State{Endpoints: []resolver.Endpoint{
			hierarchy.SetInEndpoint(resolver.NewEndpoint(wantAddrs[0]), []string{"cds:cluster_1"}),
			hierarchy.SetInEndpoint(resolver.NewEndpoint(wantAddrs[1]), []string{"cds:cluster_2"}),
			hierarchy.SetInEndpoint(resolver.NewEndpoint(wantAddrs[2]), []string{"cds:cluster_3"}),
		}},
...
func (s) TestClusterGracefulSwitch(t *testing.T) {
	...
	rrEndpoint := hierarchy.SetInEndpoint(resolver.NewEndpoint(resolver.NewAddress("rr-backend")), []string{"csp:cluster"})
	...
	pfEndpoint := hierarchy.SetInEndpoint(resolver.NewEndpoint(resolver.NewAddress("pf-backend")), []string{"csp:cluster"})
	ccs = balancer.ClientConnState{
