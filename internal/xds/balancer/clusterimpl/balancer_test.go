...
var (
	testBackendEndpoints = []resolver.Endpoint{resolver.NewEndpoint(resolver.NewAddress("1.1.1.1:1"))}
)
...
func (s) TestClusterNameInAddressAttributes(t *testing.T) {
	...
	state2 := xdsclient.SetClient(resolver.State{Endpoints: []resolver.Endpoint{resolver.NewEndpoint(addr2)}}, xdsC)
